...
namespace JF.Diagnostics
{
	...
	public sealed class ExceptionHandlerManager
	{
		...
		public event EventHandler<FailureEventArgs> Error;
		...
		public bool Handle(Exception exception, object context)
		{
			...
			{
				//构建“Error”事件参数对象
				args = new FailureEventArgs(ex, false);
			}

			//构建“Error”事件参数对象
			if(args == null)
			{
				args = new FailureEventArgs(exception, false);
			}

			//激发“Error”事件
			this.OnError(args);

			//返回事件参数对象的“ExceptionHandled”属性值
		...
		private void OnError(FailureEventArgs e)
		{
			if(this.Error != null)
			{
				this.Error(this, e);
