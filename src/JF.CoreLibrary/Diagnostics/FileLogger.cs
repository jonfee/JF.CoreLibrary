...
namespace JF.Diagnostics
{
	public abstract class FileLogger : ILogger
	{
		...
		protected virtual string GetFilePath(LogEntry entry)
		{
			...
			{
				filePath = Path.Combine(Path.GetPathRoot(JF.ComponentModel.ApplicationContextBase.Current.ApplicationDirectory), filePath.Substring(1));
			...
			{
				directoryPath = JF.ComponentModel.ApplicationContextBase.Current.EnsureDirectory(Path.GetDirectoryName(filePath.Substring(2)));
			}
			else
			{
				directoryPath = JF.ComponentModel.ApplicationContextBase.Current.EnsureDirectory(Path.GetDirectoryName(filePath));
			}
		...
		private string ResolveSequence(LogEntry entry)
		{
			...
			var infos = JF.IO.LocalFileSystem.Instance.Directory.GetFiles(System.IO.Path.GetDirectoryName(filePath), fileName.Replace(SEQUENCE, "|" + PATTERN + "|"), false);
			...
			while((index = fileName.IndexOf(SEQUENCE, index)) >= 0)
			{
				if(index > 0)
				{
					pattern += JF.IO.LocalFileSystem.LocalDirectoryProvider.EscapePattern(fileName.Substring(position, index - position));
				}
			...
			{
				pattern += JF.IO.LocalFileSystem.LocalDirectoryProvider.EscapePattern(fileName.Substring(position));
			}
