...
{
	public class LoggerModule : JF.ComponentModel.IApplicationModule, IDisposable
	{
		#region 公共属性
		...
		public virtual void Initialize(JF.ComponentModel.ApplicationContextBase context)
		{
			if(context == null)
			{
				return;
			...
			foreach(Configuration.LoggerHandlerElement handlerElement in loggerElement.Handlers)
			{
				...
				{
					typeof(Configuration.LoggerHandlerElement)
				});
				ILogger instance;
				...
				if(handlerElement.HasExtendedProperties)
				{
					...
					{
						JF.Common.Convert.SetValue(instance, property.Key, property.Value);
					}
				}
				...
				if(handlerElement.Predication != null)
				{
					...
					{
						Source = handlerElement.Predication.Source, ExceptionType = handlerElement.Predication.ExceptionType, MaxLevel = handlerElement.Predication.MaxLevel, MinLevel = handlerElement.Predication.MinLevel,
					};
				}
