...
namespace JF.ComponentModel
{
	public class ApplicationContextBase : MarshalByRefObject, INotifyPropertyChanged
	{
		...
		public Options.OptionManager OptionManager
		{
			get
			{
				return JF.Options.OptionManager.Default;
			}
		}
		...
		/// 获取当前应用程序的服务管理对象。
		/// </summary>
		public virtual JF.Services.IServiceProviderFactory ServiceFactory
		{
			get
			{
				return JF.Services.ServiceProviderFactory.Instance;
			}
		}
		...
		public virtual ICollection<IApplicationModule> Modules
		{
			get
			{
				if(_modules == null)
				{
					System.Threading.Interlocked.CompareExchange(ref _modules, new JF.Collections.Collection<IApplicationModule>(), null);
				}
