...
namespace JF.Common
{
	public static class EnumUtility
	{
		...
		public static EnumEntry GetEnumEntry(this Enum enumValue, bool underlyingType)
		{
			...
			return new EnumEntry(enumValue.GetType(), field.Name, underlyingType ? System.Convert.ChangeType(field.GetValue(null), Enum.GetUnderlyingType(enumValue.GetType())) : field.GetValue(null), alias == null ? string.Empty : alias.Alias, description == null ? string.Empty : JF.Resources.ResourceUtility.GetString(description.Description, enumValue.GetType().Assembly));
		}
		...
		public static EnumEntry[] GetEnumEntries(Type enumType, bool underlyingType, object nullValue, string nullText)
		{
			...
			for(int i = 0; i < fields.Length; i++)
			{
				...
				entries[baseIndex + i] = new EnumEntry(enumType, fields[i].Name, underlyingType ? System.Convert.ChangeType(fields[i].GetValue(null), Enum.GetUnderlyingType(enumType)) : fields[i].GetValue(null), alias == null ? string.Empty : alias.Alias, description == null ? string.Empty : JF.Resources.ResourceUtility.GetString(description.Description, enumType.Assembly));
			}
