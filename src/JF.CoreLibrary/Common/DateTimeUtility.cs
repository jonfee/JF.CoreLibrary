	...
	public static class DateTimeUtility
	{
		...
		public static bool IsDateTime(string value)
		{
			...
			if(Regex.IsMatch(value, regexDate))
			{
				...
				int iYear = Convert.ConvertValue<int>(value.Substring(0, indexY));
				int iMonth = Convert.ConvertValue<int>(value.Substring(indexY + 1, indexM - indexY - 1));
				int iDate = Convert.ConvertValue<int>(value.Substring(indexM + 1, indexD - indexM - 4));
		...
		public static int ConvertToMinute(int second)
		{
			...
			return Convert.ConvertValue<int>(Math.Ceiling(minute));
		}
		...
		public static void GetWeekRange(int year, int week, ref DateTime firstDate, ref DateTime lastDate)
		{
			...
			//当年的第一天是星期几
			int firstOfWeek = Convert.ConvertValue<int>(firstDay.DayOfWeek);
