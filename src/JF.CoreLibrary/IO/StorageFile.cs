...
namespace JF.IO
{
	...
	public class StorageFile : IStorageFile
	{
		...
		public StorageFile(JF.Services.IServiceProvider serviceProvider)
		{
			if(serviceProvider != null)
			{
				_storage = serviceProvider.Resolve<JF.Runtime.Caching.ICache>();
			}
		}

		public StorageFile(JF.Runtime.Caching.ICache storage)
		{
			if(storage == null)
			{
		...
		public void Create(StorageFileInfo file, Stream content)
		{
			...
			{
				storage.SetValue(GetFileCollectionKey(file.BucketId), new string[]
				{
					file.FileId.ToString("n")
				});
			}

			if(content != null)
			{
				var path = JF.IO.Path.Parse(file.Path);

				//确认文件的所在目录是存在的，如果不存在则创建相应的目录
				FileSystem.Directory.Create(path.Scheme + ":" + path.DirectoryName);
		...
		public Guid? Copy(Guid fileId, int bucketId)
		{
			...
			{
				Name = info.Name, Type = info.Type, Size = info.Size, Path = info.Path,
			};
		...
		public bool Move(Guid fileId, int bucketId)
		{
			...
			if(dictionary.TryGetValue("BucketId", out currentBucketId))
			{
				...
				{
					storage.SetValue(GetFileCollectionKey(bucketId), new string[]
					{
						bucketId.ToString()
					});
				}
		...
		{
			return "JF.IO.StorageFiles:" + bucketId.ToString();
		}

		[System.Runtime.CompilerServices.MethodImpl(System.Runtime.CompilerServices.MethodImplOptions.AggressiveInlining)]
		internal static string GetFileKey(Guid fileId)
		{
			return "JF.IO.StorageFile:" + fileId.ToString("n");
		}
		...
		private string GetFilePath(StorageFileInfo fileInfo)
		{
			...
			var parameters = JF.Collections.DictionaryExtension.ToDictionary<string, string>(Environment.GetEnvironmentVariables());
			...
			//返回当前文件的完整虚拟路径
			return JF.IO.Path.Combine(resolvedPath, string.Format("[{0}]{1:n}{2}", fileInfo.BucketId, fileInfo.FileId, System.IO.Path.GetExtension(fileInfo.Name).ToLowerInvariant()));
		}
