...
namespace JF.IO
{
	public static class FileSystem
	{
		...
		private static JF.Services.IServiceProvider _providers;
		...
		/// </summary>
		public static JF.Services.IServiceProvider Providers
		{
			get
			{
				if(_providers == null)
				{
					System.Threading.Interlocked.CompareExchange(ref _providers, new JF.Services.ServiceProvider(), null);
				}
		...
		public static string GetUrl(string virtualPath)
		{
			...
			if(JF.Text.TextRegular.Uri.Url.IsMatch(virtualPath))
			{
				return virtualPath;
			}
		...
		public class Matcher : JF.Services.IMatcher<string>
		{
			...
			bool JF.Services.IMatcher.Match(object target, object parameter)
			{
				var fileSystem = target as IFileSystem;
		...
		private class DirectoryProvider : IDirectory
		{
			...
			{
				Path[] paths;
				var services = FileSystem.GetDirectoryProviders(new string[]
				{
					source, destination
				}, out paths);
		...
		private class FileProvider : IFile
		{
			...
			{
				Path[] paths;
				var services = FileSystem.GetFileProviders(new string[]
				{
					source, destination
				}, out paths);
