...
namespace JF.Runtime.Caching
{
	public class BufferManager : BufferManagerBase, IDisposable
	{
		...
		private class MetadataInfo
		{
			...
			{
				return new MetadataInfo(BitConverter.ToInt64(new byte[]
				{
					(byte)'Z', (byte)'S', (byte)'B', (byte)'u', (byte)'f', (byte)'f', (byte)'e', (byte)'r'
				}, 0), 0x01000000, blockSize, blockCount);
			}
		...
		{
			name = string.IsNullOrWhiteSpace(name) ? "JF.BufferManager" : name.Trim();
