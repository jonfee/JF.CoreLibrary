...
namespace JF.Collections
{
	...
	/// </remarks>
	public class Queue : JF.Collections.IQueue
	{
		#region 静态变量
		...
		/// <summary>
		/// 初始化<seealso cref="JF.Collections.Queue"/>类的新实例，该实例为空，其初始容量为32和2.0f的成长因子。
		/// </summary>
		...
		/// <summary>
		/// 初始化<seealso cref="JF.Collections.Queue"/>类的新实例，该实例为空，具有指定的初始容量和默认为2.0f的成长因子。
		/// </summary>
		...
		/// <summary>
		/// 初始化<seealso cref="JF.Collections.Queue"/>类的新实例，该实例为空，具有指定的初始容量并使用指定的成长因子。
		/// </summary>
		/// <param name="capacity">初始化的队列容量，即包含的初始元素数。</param>
		...
		/// <summary>
		/// 移除并返回位于<see cref="JF.Collections.Queue"/>开始处的对象。
		/// </summary>
		...
		/// <summary>
		/// 将指定集合中的所有元素依次添加到<seealso cref="JF.Collections.Queue"/>的结尾处。
		/// </summary>
		...
		/// <summary>
		/// 将字符串文本添加到<seealso cref="JF.Collections.Queue"/>的结尾处。
		/// </summary>
		...
		/// <summary>
		/// 将对象添加到<seealso cref="JF.Collections.Queue"/>的结尾处。
		/// </summary>
		...
		///		<para>成长因子是当需要更大容量时当前容量要乘以的数字。在构造<seealso cref="JF.Collections.Queue"/>时确定增长因子。无论增长因子是多少，队列的容量将始终增加一个最小值（即<see cref="MinimumGrow"/>属性值），即使1.0f的增长因子也不会阻止队列的扩容。</para>
		/// </remarks>
		...
		/// <exception cref="System.InvalidOperationException">当队列为空，即<see cref="Count"/>属性等于零。</exception>
		/// <remarks>
		///		<para>此方法类似于<seealso cref="Dequeue()"/>出队方法，但本方法不修改<seealso cref="JF.Collections.Queue"/>队列。</para>
		/// </remarks>
		public object Peek()
		{
			if(_size == 0)
		...
		{
			return string.Format("{0}({1}-{2}) [{3}/{4}] {{{5}, {6}}}", _name + Environment.NewLine, _head, _tail, _size, _buffer.Length, _maximumLimit, _minimumGrow);
		}
