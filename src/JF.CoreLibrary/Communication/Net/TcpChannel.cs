...
namespace JF.Communication.Net
{
	...
	public class TcpChannel : ChannelBase
	{
		...
		public override void Send(byte[] buffer, int offset, int count, object asyncState = null)
		{
			...
			this.SendCore(() =>
			{
				return this.Packetizer.Pack(new JF.Common.Buffer(buffer, offset, count));
			}, asyncState);
		}
		...
		private void SendCore(Func<IEnumerable<JF.Common.Buffer>> pack, object asyncState)
		{
		...
		{
			if(asyncArgs.SocketError == SocketError.WouldBlock || asyncArgs.SocketError == SocketError.IOPending || asyncArgs.SocketError == SocketError.NoBufferSpaceAvailable)
			{
				//阻塞发送信号量
		...
		protected void OnReceive(SocketAsyncEventArgs asyncArgs)
		{
			...
			var buffer = new JF.Common.Buffer(asyncArgs.Buffer, asyncArgs.Offset, asyncArgs.BytesTransferred);
