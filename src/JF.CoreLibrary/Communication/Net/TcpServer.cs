...
namespace JF.Communication.Net
{
	public class TcpServer : ListenerBase
	{
		...
		private JF.Runtime.Caching.IBufferManagerSelector _bufferSelector;
		...
		{
			return new TcpServerChannelManager(this, new PacketizerFactory<TcpPacketizer>((channel) =>
			{
		...
		private void OnAcceptCompleted(SocketAsyncEventArgs asyncArgs)
		{
			...
			if(asyncArgs.SocketError == SocketError.OperationAborted || asyncArgs.SocketError == SocketError.Interrupted || asyncArgs.SocketError == SocketError.NotSocket)
			{
				return;
			}
