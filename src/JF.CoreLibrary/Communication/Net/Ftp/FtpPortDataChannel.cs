...
namespace JF.Communication.Net.Ftp
{
	internal class FtpPortDataChannel : IFtpDataChannel, IDisposable
	{
		...
		public FtpPortDataChannel(IPEndPoint address) : this(address, 1024 * 32)
		{
		}
		...
		public FtpServerChannel ServerChannel
		{
			get;
			set;
		}
		...
		protected virtual void OnReceived(byte[] buffer, int offset, int count)
		{
			...
			if(handler != null)
			{
				handler(this, buffer, offset, count);
			}
		}
		...
		private void OnConnectCompleted(object sender, SocketAsyncEventArgs e)
		{
			...
			{
				//Socket 正常关闭
				if(e.SocketError == SocketError.OperationAborted || e.SocketError == SocketError.Interrupted || e.SocketError == SocketError.NotSocket)
				{
					return;
				}
