...
namespace JF.Options.Profiles
{
	...
	/// </remarks>
	public class Profile : MarshalByRefObject, IOptionProvider, JF.Runtime.Serialization.ISerializable
	{
		#region 枚举定义
		...
		public void SetOptionObject(string path, object optionObject)
		{
			...
			else
			{
				...
				{
					section.SetEntryValue(name, JF.Common.Convert.ConvertValue<string>(optionObject));
				}
			}
		}

		void JF.Runtime.Serialization.ISerializable.Serialize(Stream serializationStream)
		{
		...
		private void UpdateEntries(ProfileSection section, object value)
		{
			...
			if(dictionary != null)
			{
				...
				{
					section.SetEntryValue(JF.Common.Convert.ConvertValue<string>(entry.Key), JF.Common.Convert.ConvertValue<string>(entry.Value));
				}

				return;
			}

			if(JF.Common.TypeExtensions.IsAssignableFrom(typeof(IDictionary<,>), value.GetType()))
			{
				foreach(var entry in (IEnumerable)value)
				{
					...
					section.SetEntryValue(JF.Common.Convert.ConvertValue<string>(entryKey), JF.Common.Convert.ConvertValue<string>(entryValue));
				}
			}
		}
