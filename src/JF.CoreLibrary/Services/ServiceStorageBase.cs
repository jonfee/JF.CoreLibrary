...
namespace JF.Services
{
	public abstract class ServiceStorageBase : MarshalByRefObject, IServiceStorage, IEnumerable<ServiceEntry>
	{
		...
		protected ServiceStorageBase(IServiceProvider provider) : this(provider, JF.Services.Matcher.Default)
		{
		}
		...
		public virtual ServiceEntry Get(string name)
		{
			...
			var result = this.Find(name, new List<IServiceStorage>(new[]
			{
				this
			}));
			...
			{
				result = this.Find(name, new List<IServiceStorage>(new[]
				{
					ServiceProviderFactory.Instance.Default.Storage
				}));
			}
		...
		protected virtual bool OnMatch(ServiceEntry entry, object parameter)
		{
			...
			var matcher = this.Matcher ?? JF.Services.Matcher.Default;
		...
		protected virtual object Find(Type type, object parameter, bool isMultiplex)
		{
			...
			var result = Find(type, parameter, isMultiplex, new List<IServiceStorage>(new[]
			{
				this
			}));
			...
			{
				result = this.Find(type, parameter, isMultiplex, new List<IServiceStorage>(new[]
				{
					ServiceProviderFactory.Instance.Default.Storage
				}));
			}
