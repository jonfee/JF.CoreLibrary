...
namespace JF.Services
{
	public class CommandOptionCollection : IEnumerable<KeyValuePair<string, string>>
	{
		...
		public bool Bind(ICommand command)
		{
			...
			var descriptor = Cache.GetOrAdd(command.GetType(), type => new CommandDescriptor()
			{
				Metadata = (CommandAttribute)Attribute.GetCustomAttribute(type, typeof(CommandAttribute)), Options = Attribute.GetCustomAttributes(type, typeof(CommandOptionAttribute), true).Cast<CommandOptionAttribute>()
			});
