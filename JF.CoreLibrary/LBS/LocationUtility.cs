...
namespace JF.CoreLibrary.LBS
{
    ...
    public class LocationUtility
    {
        ...
        public static LocationRange GetPosition(Location location, double distance)
        {
            if (location == null)
        ...
        /// <param name="distance">半径(公里)</param>
        /// <returns>指定半径外的四个顶点。</returns>
        public static LocationRange GetPosition(double latitude, double longitude, double distance)
        {
        ...
        public static bool IsInRange(Location source, Location target, double distance)
        {
            ...
            // 计算目标位置区域的四个顶点。
            var range = LocationUtility.GetPosition(target, distance);

            return source.Latitude < range.LeftTop.Latitude && source.Latitude > range.RightBottom.Latitude && source.Longitude > range.LeftTop.Longitude && source.Longitude < range.RightBottom.Longitude;
        }
