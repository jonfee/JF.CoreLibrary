...
namespace JF.CoreLibrary.LBS
{
    public class LocationConverter : TypeConverter
    {
        ...
        private const double PI = 3.14159265358979324;      //圆周率
        private const double X_PI = PI * 3000.0 / 180.0;    //圆周率转换量
        private const double EE = 0.00669342162296594323;   //第一偏心率平方，即：椭球的偏心率
        private const double AA = 6378245.0;                //长半轴，即：卫星椭球坐标投影到平面地图坐标系的投影因子
        private const double BB = 6356863.0188;             //短半轴
        ...
        private static Location EncryptGCJ02(double latitude, double longitude)
        {
            ...
            if (IsOutOfChina(latitude, longitude))
                return new Location(latitude, longitude, series: LocationSeries.WGS84);
            ...
            //返回结果。
            return new Location(latitude + bufferLatitude, longitude + bufferLongitude, series: LocationSeries.GCJ02);
        }
        ...
        private static Location DecryptGCJ02(double latitude, double longitude)
        {
            ...
            //返回结果。
            return new Location(latitude - latitude1, longitude - longitude1, series: LocationSeries.WGS84);
        }
        ...
        private static Location EncryptBD09(double latitude, double longitude)
        {
            ...
            //返回结果。
            return new Location(latitude1, longitude1, series: LocationSeries.BD09);
        }
        ...
        private static Location DecryptBD09(double latitude, double longitude)
        {
            ...
            //返回结果。
            return new Location(latitude1, longitude1, series: LocationSeries.GCJ02);
        }
